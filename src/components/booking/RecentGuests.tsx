...
// Mock recent guests data - sorted by recency
...
const RecentGuests = ({ onSelect }: RecentGuestsProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<string>("Hammasi");
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="w-full flex items-center gap-2 px-4 py-3 rounded-xl border border-dashed border-primary/40 bg-primary/5 text-primary font-semibold text-sm transition-all active:scale-[0.98] hover:bg-primary/10"
      >
        <Users className="w-4 h-4" />
        Oldingi mehmonlardan tanlash
      </button>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden animate-fade-in">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/50">
        ...
        <button type="button" onClick={() => setIsOpen(false)} className="p-1 rounded-md hover:bg-muted">
      ...
      <div className="max-h-48 overflow-y-auto">
        {filtered.length === 0 ? (
          <p className="text-center text-xs text-muted-foreground py-4">Mehmon topilmadi</p>
        ) : (
          filtered.map((guest) => (
            <button
              key={guest.phone}
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted/60 active:bg-muted transition-colors text-left border-t border-border/50 first:border-t-0"
            >
              <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                <span className="text-sm font-bold text-primary">{guest.name[0]}</span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-foreground truncate">{guest.name}</span>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>·</span>
                  <span>{guest.price.toLocaleString()} so'm</span>
                </div>
                {guest.hostel && (
                  <span className="text-[10px] text-muted-foreground/70">{guest.hostel} · {guest.room}</span>
                )}
              </div>
            </button>
          ))
