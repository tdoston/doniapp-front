...
const Index = () => {
  ...
  return (
    ...
    >
      ...
      {isFullRoom && (
        <div className="bg-card border-b border-border px-2 py-2.5 shrink-0">
          {/* Guest count summary */}
          <div className="flex items-center justify-between px-2 mb-2">
            <span className="text-xs font-semibold text-muted-foreground">
              Mehmonlar: {guests.length} nafar
            </span>
            <button
              onClick={addGuest}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-primary text-primary-foreground shadow-sm active:scale-[0.96] transition-all"
            >
          ...
          {/* Scrollable guest chips */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none">
            {guests.map((g, idx) => {
              const isActive = idx === activeGuestIdx;
              const hasData = !!(g.phone || g.price);
              return (
                <button
                  key={g.id}
                  onClick={() => setActiveGuestIdx(idx)}
                  className={`relative flex items-center gap-1 min-w-[72px] px-3 py-2.5 rounded-2xl text-xs font-bold whitespace-nowrap transition-all active:scale-[0.96] ${
                    isActive
                ...
                >
                  ...
                  <span className="flex flex-col items-start leading-tight">
                    <span className="text-[11px]">
                      {g.phone ? g.phone.slice(-4) : "Bo'sh"}
                    </span>
                    {g.price && (
                      <span className="text-[9px] opacity-60">
                        {Number(g.price).toLocaleString("uz-UZ")}
                      </span>
                    )}
                  </span>
                  {guests.length > 1 && isActive && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        removeGuest(idx);
                      }}
                      className="ml-0.5 p-1 rounded-full bg-primary-foreground/20 text-primary-foreground hover:bg-destructive/80 hover:text-destructive-foreground transition-colors"
                    >
